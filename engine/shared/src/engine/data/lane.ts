...
// projection gives x(y) = x(judgmentY) * (1 + k * (y - judgmentY)).
// This preserves off-centre beam lean; the baked image already contains the
// effect's own perspective. Apply only the lane-centre translation shear.
export const nativeEffectSkew =
    (nativeStage.cameraPitchSin * nativeStage.tanHalfVerticalFov * (horizonY - judgmentY)) /
    (nativeStage.cameraPitchCos + nativeStage.cameraPitchSin * nativeStage.tanHalfVerticalFov * judgmentY)
